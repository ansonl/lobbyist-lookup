function Filing(organization, client, houseid, senateid, lobbyistArray) {
	if (organization == '')
		organization = 'No organization on file';
	if (client == '')
		client = 'No client on file';
	if (houseid == '')
		houseid = 'No House ID on file';
	if (senateid == '')
		senateid = 'No Senate ID on file';
...
function createFiling(data) {
	...
	*/
	...
	if (data.Lobbyist != null) {
		...
		lobbyists.push(new Lobbyist('No lobbyists on file',''));
	}
